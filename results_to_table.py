...
def fill_in_latex_table(filepath, outpath, metric_index):
    ...
    with open(f"{outpath}/{get_metric_name(metric_index)}.txt", 'w') as out:
        ...
        out.write("\n                                & \multicolumn{4}{c}{Descriptor} \\\\")
        out.write("\n\cline{2-5}")
        out.write("\nDetectors                       & \\textit{AKAZE}  & \\textit{BRIEF}   & \\textit{ORB}    & \\textit{SIFT}      \\\\")
        out.write("\n\hline")
        out.write("\n\\textit{Shi-Tomasi}   " + f"   & N/A              & {shi[0]}          & {shi[1]}         & {shi[2]}       " + "\\\\")
        out.write("\n\\textit{SIFT}         " + f"   & N/A              & {sift[0]}         & N/A              & {sift[1]}      " + "\\\\")
        out.write("\n\\textit{CenSurE}      " + f"   & N/A              & {censure[0]}      & {censure[1]}     & {censure[2]}   " + "\\\\")
        out.write("\n\\textit{AKAZE}        " + f"   & {akaze[0]}       & {akaze[1]}        & {akaze[2]}       & {akaze[3]}     " + "\\\\")
        out.write("\n\\textit{ORB}          " + f"   & N/A              & {orb[0]}          & {orb[1]}         & {orb[2]}       " + "\\\\")
        out.write("\n\end{tabular}")
        out.write("\n\end{table}")
